...
def Xattention_prefill(
    ...
    key_states = key_states.transpose(1, 2).reshape(
        batch_size * k_len, num_heads, head_dim
    )
...
def prefill_xattn(query_states, key_states, value_states, threshold, causal):
    ...
    key_states = (
        key_states[:, :, None, :, :]
        .expand(bsz, num_kv_heads, num_kv_groups, seq_len, head_dim)
    ...
    value_states = (
        value_states[:, :, :, None, :]
        .expand(bsz, seq_len, num_kv_heads, num_kv_groups, head_dim)
