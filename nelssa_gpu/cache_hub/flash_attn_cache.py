...
class flash_attn_cache(KV_Cache):
    ...
    def __init__(
        ...
        self.valid_start_list = (
            valid_start  # start index of valid tokens for each batch
        )
        ...
        for device_idx in self.device_list:
            self.batch_indices_dict[device_idx] = torch.arange(
                self.batch_size, dtype=torch.int32, device=device_idx
            )
        ...
        if self.allocated:
            self.key_cache = [
                torch.zeros(
                    (self.batch_size, self.max_length, self.kv_head, self.head_dim),
    ...
    def prefill_update_kv_cache(
        ...
        if self.valid_length is None:
            temp_valid_length = torch.from_numpy(seq_len - self.valid_start_list).to(
                torch.int32
            )
            for device_idx in self.device_list:
        ...
        with torch.cuda.stream(self.copystream):
            ...
            ].wait()  # wait for KV ready
            self.key_cache[layer_idx][
                start_bdx : start_bdx + bsz, :_valid_length, :, :
            ].copy_(key_states[:, _valid_start:, :, :], non_blocking=True)
            self.value_cache[layer_idx][
                start_bdx : start_bdx + bsz, :_valid_length, :, :
            ].copy_(value_states[:, _valid_start:, :, :], non_blocking=True)
    ...
    def sync(self, layer_idx, start_bdx):
        self.copyevents[
            self.layer_mapping[str(layer_idx)]
        ].synchronize()  # wait for copy done
    ...
    def decode_update_kv_cache(self, key_states, value_states, layer_idx):
        ...
        Args:
            ...
            layer_idx: the index of the layer
        """
        self.key_cache[layer_idx][self.batch_indices, self.valid_length, :, :] = (
            key_states[:, 0, :, :]
        )
        self.value_cache[layer_idx][self.batch_indices, self.valid_length, :, :] = (
            value_states[:, 0, :, :]
        )
        ...
        if layer_idx == self.layer_num - 1:
            ...
            self.valid_length = self.valid_length_dict[
                self.layer_mapping[str(layer_idx)]
            ]
