...
class retroinfer_cache_gpu(KV_Cache):
    ...
    def __init__(
        ...
        self.BUILD_SEGMENT = (
            16384 if self.batch_size < 8 else 8192
        )  # min length to build index when prefilling
        self.BUILD_CENTROIDS = max(
            round(self.BUILD_SEGMENT / (avg_cluster_size * 8)) * 8, 8
        )  # must be divisible by 8
        self.BUILD_NPROBE = max(
            round(self.BUILD_CENTROIDS * retrieval_budget), 1
        )  # build retrieve zone size per segment
        self.BUILD_ES = max(
            round(self.BUILD_CENTROIDS * estimation_budget), 1
        )  # build estimation zone size per segment
        ...
        assert actual_gen_len >= 0, (
            f"Decoding generation length({actual_gen_len}) should >= 0"
        )
        if self.input_length <= 0:
            ...
            # update threshold
            self.THRESHOLD = (
                self.static_pattern_start + self.static_pattern_end + self.BUILD_SEGMENT
            )
            ...
            if self.will_update_index:
                ...
                # will not update index after built the first index
                    ...
                    self.static_stride = (
                        self.input_length + actual_gen_len - self.BUILD_SEGMENT
                    )
        ...
        self.execution_stride = (
            self.static_stride + self.buffer_size
        )  # execution buffer size
        print(
            f"Buffer size: {self.buffer_size}, Steady zone size: {self.static_stride}"
        )
    ...
    def pre_allocate_decision(self):
        ...
        self.esitimate_gpu_memory = (
            ...
            * self.batch_size
            * self.kv_head
            * (self.n_centroids + self.static_stride)
            * self.head_dim
            * 2
        )
        self.esitimate_gpu_memory += (
        ...
        return (
            self.free_memory > self.esitimate_gpu_memory * 1.5,
    ...
    def prepare_cache(self):
        ...
        if self.offload_kv:  # move offloaded keys & values to GPU
            for ldx in range(self.layer_num):
                self.list_keys[ldx] = (
                    self.list_keys[ldx].to(self.layer_mapping[str(ldx)]).contiguous()
                )
                self.list_values[ldx] = (
                    self.list_values[ldx].to(self.layer_mapping[str(ldx)]).contiguous()
                )
    ...
    def prefill_update_kv_cache(
        ...
        assert seq_len <= self.input_length, (
            f"seq_len({seq_len}) should less than input_length({self.input_length})"
        )
        ...
        if self.build_index_when_prefilling:
            ...
            assert valid_start == 0, (
                f"Requests in the same batch should have the same length."
            )
            self.mainevents[self.layer_mapping[str(layer_idx)]].record()
            # async copy KV cache
            with torch.cuda.stream(self.copystream):
                ...
                for batch_idx in range(bsz):  # loop to preserve pinned for fast copy
                    for head_idx in range(self.kv_head):
                        self.list_keys[layer_idx][
                            start_bdx + batch_idx, head_idx, :seq_len, :
                        ].copy_(
                            key_states[batch_idx, :, head_idx, :], non_blocking=True
                        )
                        self.list_values[layer_idx][
                            start_bdx + batch_idx, head_idx, :seq_len, :
                        ].copy_(
                            value_states[batch_idx, :, head_idx, :], non_blocking=True
                        )
                self.copyevents[self.layer_mapping[str(layer_idx)]].record()
        ...
        )  # ignore mask tokens, shape: (bsz, seq_len, group_num, dim)
        ...
        for ldx in range(self.layer_num):
            ...
            if self.index_built:
                ...
                update_keys = (
                    ...
                    .clone()
                    .reshape(self.batch_groups, self.BUILD_SEGMENT, self.head_dim)
                    .contiguous()
                )
                update_values = (
                    self.list_values[ldx][
                ...
                # copy steady zone
                self.steady_zone_keys[ldx][:, :, : self.static_pattern_start, :] = (
                    self.list_keys[ldx][:, :, : self.static_pattern_start, :]
                )
                self.steady_zone_values[ldx][:, :, : self.static_pattern_start, :] = (
                    self.list_values[ldx][:, :, : self.static_pattern_start, :]
                )
                self.steady_zone_keys[ldx][
                    :,
                    :,
            ...
            _centroids, _value_sum, _clusters, _cluster_size = segment_k_means(
                key=update_keys
                - mean_key,  # centering to 0, (batch_size*group_num, SEGMENT, dim)
                value=update_values,  # (batch_size*group_num, SEGMENT, dim)
                num_centroids=(
                    self.UPDATE_CENTROIDS if self.index_built else self.BUILD_CENTROIDS
                ),
                num_segments=1,
            )
            _centroids += mean_key
            ...
            )  # (batch_size*group_num, new_n_centroids, dim)
            ...
            )  # (batch_size*group_num, new_n_centroids)
            ...
            if self.n_centroids > 0:
                new_cluster_cumsum += self.cluster_size_cumsum[ldx][
                    :, -1:
                ]  # (batch_size*group_num, 1)
            self.cluster_size_cumsum[ldx] = torch.cat(
                (self.cluster_size_cumsum[ldx], new_cluster_cumsum), dim=1
            )  # (batch_size*group_num, new_n_centroids)
    ...
    def dense_attention(self, queries, layer_idx, static_len):
        ...
        self.list_keys[layer_idx][:, :, static_len - 1, :] = self.new_key_states[
            :, 0, :, :
        ]
        self.list_values[layer_idx][:, :, static_len - 1, :] = self.new_value_states[
            :, 0, :, :
        ]
        ...
        attn_out = flash_attn_with_kvcache(
            ...
            k_cache=self.list_keys[layer_idx].view(
                self.batch_groups, self.list_stride, 1, self.head_dim
            ),
            v_cache=self.list_values[layer_idx].view(
                self.batch_groups, self.list_stride, 1, self.head_dim
            ),
    ...
    def sparse_attention_gpu(self, queries, layer_idx, static_len):
        ...
        self.steady_zone_keys[layer_idx][:, :, static_len - 1, :] = self.new_key_states[
            :, 0, :, :
        ]
        self.steady_zone_values[layer_idx][:, :, static_len - 1, :] = (
            self.new_value_states[:, 0, :, :]
        )
        self.new_key_states = None
        self.new_value_states = None
