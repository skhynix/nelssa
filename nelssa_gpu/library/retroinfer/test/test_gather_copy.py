...
def gen_two_indices(rows, cols, max_range1, max_range2, unit_size):
    ...
    src_copy_size1 = np.random.randint(
        1, unit_size + 1, size=(rows, cols), dtype=np.int32
    )
    ...
    src_copy_size2 = np.random.randint(
        1, unit_size + 1, size=(rows, cols), dtype=np.int32
    )
...
def test_concat_gather_copy():
    ...
    copy_vector_num_tensor = torch.tensor(
        copy_vector_num, dtype=torch.int32, device="cuda"
    )

    key_src1 = torch.randn(
        (groups, src_vector_num1, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    key_src2 = torch.randn(
        (groups, src_vector_num2, dim), pin_memory=True, dtype=DTYPE
    ).contiguous()
    key_src3 = torch.randn(
        (groups, src_unit_num3, unit_size, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    key_dst1 = torch.randn(
        (groups, buffer_vector_num, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    key_dst2 = key_dst1.clone()

    value_src1 = torch.randn(
        (groups, src_vector_num1, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    value_src2 = torch.randn(
        (groups, src_vector_num2, dim), pin_memory=True, dtype=DTYPE
    ).contiguous()
    value_src3 = torch.randn(
        (groups, src_unit_num3, unit_size, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    value_dst1 = torch.randn(
        (groups, buffer_vector_num, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    ...
    (
        ...
        groups, index_length, src_vector_num2 - unit_size, src_unit_num3, unit_size
    )
    ...
    for i in range(groups):
        ...
        for j in range(copy_chunks1[i]):
            key_dst2[i, copy_num : copy_num + src_copy_size1[i, j], :] = key_src2[
                i, src_indices1[i, j] : src_indices1[i, j] + src_copy_size1[i, j], :
            ]
            value_dst2[i, copy_num : copy_num + src_copy_size1[i, j], :] = value_src2[
                i, src_indices1[i, j] : src_indices1[i, j] + src_copy_size1[i, j], :
            ]
            copy_num += src_copy_size1[i, j]

        for j in range(copy_chunks2[i]):
            key_dst2[i, copy_num : copy_num + src_copy_size2[i, j], :] = key_src3[
                i, src_indices2[i, j], : src_copy_size2[i, j], :
            ]
            value_dst2[i, copy_num : copy_num + src_copy_size2[i, j], :] = value_src3[
                i, src_indices2[i, j], : src_copy_size2[i, j], :
            ]
...
def gen_indices(rows, cols, max_range1, max_range2, unit_size):
    ...
    src_copy_size = np.random.randint(
        1, unit_size + 1, size=(rows, cols), dtype=np.int32
    )
...
def test_gather_copy_scatter():
    ...
    key_src = torch.randn(
        (groups, src_unit_num * unit_size, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    key_dst1 = torch.randn(
        (groups, dst_unit_num, unit_size, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    key_dst2 = key_dst1.clone()

    value_src = torch.randn(
        (groups, src_unit_num * unit_size, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    value_dst1 = torch.randn(
        (groups, dst_unit_num, unit_size, dim), device="cuda", dtype=DTYPE
    ...
    src_indices, src_copy_size, dst_indices, copy_chunks = gen_indices(
        groups,
        index_length,
        src_unit_num * unit_size - copy_start,
        dst_unit_num,
        unit_size,
    )
    torch.cuda.synchronize()
    ...
    for i in range(groups):
        ...
        for j in range(copy_chunks[i]):
            key_dst2[i, dst_indices[i, j], : src_copy_size[i, j], :] = key_src[
                i,
                copy_start + src_indices[i, j] : copy_start
                + src_indices[i, j]
                + src_copy_size[i, j],
                :,
            ]
            value_dst2[i, dst_indices[i, j], : src_copy_size[i, j], :] = value_src[
                i,
                copy_start + src_indices[i, j] : copy_start
                + src_indices[i, j]
                + src_copy_size[i, j],
...
def test_gather_copy_vectors():
    ...
    key_src = torch.randn(
        (groups, src_vector_num, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    key_dst1 = torch.randn(
        (groups, buffer_size, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    key_dst2 = key_dst1.clone()
    ...
    src_metadata = torch.randn(
        size=(groups, src_vector_num), dtype=DTYPE, device="cuda"
    ).contiguous()
    dst_metadata1 = torch.empty(
        (groups, buffer_size), dtype=DTYPE, device="cuda"
    ).contiguous()
    ...
    for i in range(groups):
        indices[i, :] = (
            torch.randperm(src_vector_num)[:index_size].to(torch.int64).to("cuda")
        )
...
def test_reorganize_vectors():
    ...
    copy_start = torch.randint(
        0, n_centroids - copy_cluster_num + 1, (1,), dtype=torch.int32
    )[0].item()
    # copy_start = 0
    ...
    for i in range(groups):
        cluster_sizes[i] = split_integer_sum(dst_vector_num, n_centroids).to(
            dtype=torch.int32, device="cuda"
        )

    cluster_cumsum = torch.cumsum(cluster_sizes, dim=1, dtype=torch.int32)
    assert (cluster_cumsum[:, -1] == dst_vector_num).all(), (
        f"{cluster_cumsum[:, -1]} != {dst_vector_num}"
    )
    ...
    for i in range(groups):
        ...
        for j in range(copy_cluster_num):
            max_cluster_size = max(
                max_cluster_size, cluster_sizes[i, copy_start + j].item()
            )
    ...
    print(
        max_cluster_size,
        src_vector_num,
        (cluster_sizes[:, copy_start : copy_start + copy_cluster_num] == 0).any(),
    )
    ...
    key_dst1 = torch.randn(
        (groups, dst_vector_num, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    key_dst2 = key_dst1.clone()
    ...
    clusters = torch.randint(
        -100,
        2 * src_vector_num,
        (groups, copy_cluster_num, max_cluster_size),
        dtype=torch.int32,
        device="cuda",
    )
    for i in range(groups):
        random_indices = torch.randperm(
            src_vector_num, dtype=torch.int32, device="cuda"
        )
        start_idx = 0
        for j in range(copy_cluster_num):
            cluster_size = cluster_sizes[i, copy_start + j].item()
            clusters[i, j, :cluster_size] = random_indices[
    ...
    for i in range(groups):
        for j in range(copy_cluster_num):
            ...
            for k in range(copy_cluster_size):
                ...
                value_dst2[i, copy_start_index + k, :] = value_src[
                    i, clusters[i, j, k], :
                ]
    torch.cuda.synchronize()
...
def test_gather_copy_cluster_and_concat_fuse(nprobe=70):
    ...
    cluster_sizes = torch.empty((groups, n_centroids), dtype=torch.int32, device="cuda")
    for i in range(groups):
        cluster_sizes[i] = split_integer_sum(src_vector_num2, n_centroids).to(
            dtype=torch.int32, device="cuda"
        )
    cluster_cumsum = torch.cumsum(cluster_sizes, dim=-1, dtype=torch.int32)
    ...
    select_indices = torch.randint(
        0, n_centroids, (groups, index_size), dtype=torch.int64, device="cuda"
    )
    ...
    key_src2 = torch.randn(
        (groups, src_vector_num2, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    key_dst1 = torch.randn(
        (groups, buffer_vector_num, dim), device="cuda", dtype=DTYPE
    ).contiguous()
    ...
    for i in range(groups):
        ...
        for j in range(nprobe):
            ...
            start_idx = (
                0 if cluster_id == 0 else cluster_cumsum[i, cluster_id - 1].item()
            )
            ...
            key_dst2[i, copy_num : copy_num + copy_size, :] = key_src2[
                i, start_idx : start_idx + copy_size, :
            ]
            value_dst2[i, copy_num : copy_num + copy_size, :] = value_src2[
                i, start_idx : start_idx + copy_size, :
            ]
            copy_num += copy_size
