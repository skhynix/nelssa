...
class QwenLayer:
    ...
    def init_layer(self, hf_qwen_layer):
        ...
        self.bqkv = torch.cat((self.bq, self.bk, self.bv), dim=0).to(
            self.device, non_blocking=True
        )
...
class QwenModel(LLM):
    ...
    def _set_cos_sin_cache(self):
        if self.max_length > 32768:
            ...
            pos_freqs = self.base ** (
                torch.arange(0, self.head_dim, 2).float().to(self.inv_freq.device)
                / self.head_dim
            )
    ...
    def init_model(self):
        ...
        if self.model_name == "Qwen/Qwen2.5-7B-Instruct":
            self.thresholds = [
                torch.tensor(qwen_25_7b_8_thresholds[layer_idx]).to(
            ...
            self.thresholds = [
                torch.tensor(qwen_25_72b_8_thresholds[layer_idx]).to(
                    self.layer_mapping[str(layer_idx)]
                )
                for layer_idx in range(self.num_layers)
            ]
    ...
    def init_kv_cache(self, valid_start, attn_config, nelssa_client=None):
        ...
        if self.attention_type == "Full_Flash_Attn":
            ...
            nelssa_config = qwen_config.get("NELSSA")
