...
def parse_args():
    ...
    parser.add_argument(
        "--chunk_size", type=int, default=-1, help="Chunk size for chunked prefill"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    sampling_params = SamplingParams(
        n=1, max_tokens=args.gen_len, ignore_eos=True, temperature=0.0
    )
    ...
    for output in outputs:
        print(
            f"arrival time: {output.metrics.arrival_time}, first_token_time: {output.metrics.first_token_time}, finished_time: {output.metrics.finished_time}"
        )
