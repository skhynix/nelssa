...
# Todo: preprocess (initialization)
@config
class NELSSAConfig:
    """Configuration for NELSSA sparse KV caching with CPU offloading.

    Stores only the first N and last M tokens' KV cache on GPU, offloading
    the intermediate tokens to CPU. This is useful for long context sequences
    where full KV cache would exceed GPU memory.
    """
    ...
    nprobe: int = 10  # Number of clusters to retrieve (retrieval zone size)
    es_cluster_num: int = 5  # Number of clusters for estimation zone
    retrieval_budget: float = 0.018  # Ratio for retrieval zone (match RetroInfer's retrieval_budget default; NELSSA has no estimation zone)
    estimation_budget: float = 0.232  # Ratio for estimation zone (default from nelssa)

    # CPU parallel attention for decode phase
    enable_cpu_attention: bool = False  # Enable CPU attention computation in parallel
    cpu_attention_kv_fraction: float = 1.0  # Fraction of offloaded KV to use for CPU attention

    # Offloading control
    enable_offloading: bool = True  # Enable KV offloading to CPU (disable for testing)

    # NELSSA: Max Long Requests
    max_num_long_requests: int = 8 

    def nelssa_check(self, prompt_length: int) -> bool:
        return self.enabled and prompt_length >= self.prompt_length_threshold

    def get_gpu_tokens_count(self) -> int:
        return self.first_n + self.last_m

    def get_cpu_buffer_size(self, seq_len: int) -> int:
        gpu_tokens = self.get_gpu_tokens_count()
        return max(0, seq_len - gpu_tokens)
...
class CacheConfig:
    ...
    """Configuration for NELSSA sparse KV caching with CPU offloading. When enabled,
    only the first N and last M tokens' KV cache are stored on GPU, with
    intermediate tokens offloaded to CPU."""
