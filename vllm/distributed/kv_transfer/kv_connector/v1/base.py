...
class KVConnectorBase_V1(ABC):
    ...
    def register_nelssa_metadata_buffers(
        ...
        """Register NELSSA clustering-metadata staging buffers"""
        return

    def set_nelssa_metadata_store_callback(self, callback) -> None:
        """Register a callback to store received NELSSA metadata"""
        return

    def set_nelssa_meta_slot_free_callback(self, callback) -> None:
        """Register a callback to notify when NELSSA metadata NIXL READ
        completes, so the runner can send slot_free RPC to the P-side."""
        return
