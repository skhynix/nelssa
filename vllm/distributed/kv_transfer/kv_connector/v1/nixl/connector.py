...
from vllm.distributed.kv_transfer.kv_transfer_state import pop_nelssa_meta_slot
...
class NixlConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def request_finished(
        ...
        assert self.connector_scheduler is not None
        return self._request_finished(request, (block_ids,))
    ...
    def _request_finished(
        ...
        delay_free, result = self.connector_scheduler.request_finished(request, block_ids)

        # Deliver Staging Slot info with request_finished() to Decode Worker
        if result is not None and result.get("do_remote_prefill"):
            staged = pop_nelssa_meta_slot(request.request_id)
            if staged is not None:
                result["remote_slot_idx"] = staged[0]
                result["remote_n_centroids"] = staged[1]
        return delay_free, result
    ...
    def register_nelssa_metadata_buffers(
        ...
        assert self.connector_worker is not None
        self.connector_worker.register_nelssa_metadata_buffers(centroid_buf, cluster_size_buf)

    def set_nelssa_metadata_store_callback(self, callback) -> None:
        assert self.connector_worker is not None
        self.connector_worker.set_nelssa_metadata_store_callback(callback)

    def set_nelssa_meta_slot_free_callback(self, callback) -> None:
        assert self.connector_worker is not None
        self.connector_worker.set_nelssa_meta_slot_free_callback(callback)
