...
class NixlAgentMetadata:
    ...
    # - Base addresses of the two registered NIXL regions
    # - per-slot byte stride for building remote transfer descriptors (decode)
    nelssa_centroid_base_addrs: list[int] = field(default_factory=list)
    nelssa_cluster_size_base_addrs: list[int] = field(default_factory=list)
    nelssa_centroid_slot_bytes: int = 0
    nelssa_max_n_centroids: int = 0
...
class RemoteMeta:
    ...
    # NELSSA: Cluster Metadata Transfer
    # Request Info for NIXL READ: slot index in staging buffer & n_centroids for the request
...
class NixlConnectorMetadata(KVConnectorMetadata):
    ...
    def add_new_req_to_recv(
        ...
        req.remote = RemoteMeta(
            ...
            slot_idx=kv_transfer_params.get("remote_slot_idx"),
            n_centroids=kv_transfer_params.get("remote_n_centroids"),
        )
