...
class NixlConnectorScheduler:
    ...
    def update_state_after_alloc(
        ...
        if params.get("do_remote_decode"):          # Prefill Worker
            self._reqs_in_batch.add(request.request_id)
        if self.use_host_buffer and params.get("do_remote_decode"):
            # NOTE: when accelerator is not directly supported by Nixl,
            # prefilled blocks need to be saved to host memory before transfer.
            self._reqs_need_save[request.request_id] = request
        elif params.get("do_remote_prefill"):       # Decode Worker
            if params.get("remote_block_ids"):
                if all(
                    ...
                    # NELSSA: Filter middle blocks of long request
                    if self._check_nelssa_filter(request):
                        local_block_ids = self._filter_middle_blocks(request, local_block_ids)
    ...
    # NELSSA: Long KV Filtering (Head + Tail)
    def _check_nelssa_filter(self, request: "Request") -> bool:
        params = request.kv_transfer_params
        if not params:
            return False
            
        if not (hasattr(self.vllm_config.cache_config, 'nelssa_config') and
            getattr(self.vllm_config.cache_config.nelssa_config, 'enabled', False)):
            return False

        do_remote_prefill = params.get("do_remote_prefill", False)
        is_long = getattr(request, 'nelssa_long_mask', False)
        apply = do_remote_prefill and is_long
        return apply

    def _filter_middle_blocks(
        self, request: "Request", block_id_groups: BlockIds
    ) -> BlockIds:
        first_n, last_m = 16, 64
        block_size = self.block_size

        prompt_len = request.num_prompt_tokens
        head_blocks = (first_n + block_size - 1) // block_size
        if prompt_len > last_m:
            tail_start = (max(prompt_len - last_m, 0) // block_size) * block_size
            tail_tokens = max(prompt_len - tail_start, 0)
            tail_blocks = (tail_tokens + block_size - 1) // block_size
        else:
            tail_blocks = (last_m + block_size - 1) // block_size

        filtered_groups = []
        for group_blocks in block_id_groups:
            if len(group_blocks) <= head_blocks + tail_blocks:
                filtered_groups.append(group_blocks)
            else:
                head_block_ids = list(group_blocks[:head_blocks])
                tail_block_ids = list(group_blocks[-tail_blocks:])
                filtered_groups.append(tuple(head_block_ids + tail_block_ids))
    ...
    def build_connector_meta(
        ...
        for req_id, (req, block_ids) in self._reqs_need_recv.items():
            assert req.kv_transfer_params is not None
            logger.info(
                "[NIXL][NELSSA][META] build_connector_meta: req=%s, "
                "do_remote_prefill=%s, remote_slot_idx=%s, remote_n_centroids=%s",
                req_id,
                req.kv_transfer_params.get("do_remote_prefill"),
                req.kv_transfer_params.get("remote_slot_idx"),
                req.kv_transfer_params.get("remote_n_centroids"),
            )
    ...
    def request_finished(
        ...
        result = dict(
            ...
            remote_slot_idx=params.get("remote_slot_idx"),          # Staging Slot Index
            remote_n_centroids=params.get("remote_n_centroids"),    # Valid Cluster Number
        )
