...
class NixlConnectorWorker:
    ...
    def __init__(
        ...
        # Track Clustering Metadata transfers (centroid handle + cluster_size handle)
        self._recving_meta_transfers: defaultdict[ReqId, list[TransferHandle]] = defaultdict(list)

        # Backup Cluster Metadata Info (slot_idx, n_centroids) to avoid race condition with KV block READ
        self._nelssa_meta_recv_info: dict[ReqId, tuple[int, int]] = {}
        ...
        self._nelssa_cluster_transfer_enabled = False               # Check whether NIXL Memory regions are registered
        self._nelssa_centroid_buf: torch.Tensor | None = None
        self._nelssa_cluster_size_buf: torch.Tensor | None = None
        self._nelssa_registered_descs: list | None = None
        
        self._nelssa_centroid_base_addrs: list[int] = []            # Handshake Metadata (NixlAgentMetadata)
        self._nelssa_cluster_size_base_addrs: list[int] = []
        self._nelssa_centroid_slot_bytes: int = 0
        self._nelssa_cluster_size_slot_bytes: int = 0

        # Remote peer's metadata buffer geometry, keyed by engine_id.
        # Each entry: (centroid_base_addrs, cluster_size_base_addrs,
        #              centroid_slot_bytes, max_n_centroids, device_id)
        # Only the centroid slot stride is carried over the handshake; the
        # cluster_size stride is derived from the local buffer geometry (the
        # two sides allocate identical-layout staging buffers, so they match).
        self._nelssa_meta_remote: dict[EngineId, tuple] = {}

        self._nelssa_max_n_centroids: int = 0
        self._nelssa_store_metadata_callback: Callable | None = None    # Store the transferred Cluster Metadata in Decode Worker Buffer
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        ...
        agent_metadata = NixlAgentMetadata(
            ...
            nelssa_centroid_base_addrs=getattr(self, "_nelssa_centroid_base_addrs", []),
            nelssa_cluster_size_base_addrs=getattr(self, "_nelssa_cluster_size_base_addrs", []),
            nelssa_centroid_slot_bytes=getattr(self, "_nelssa_centroid_slot_bytes", 0),
            nelssa_max_n_centroids=getattr(self, "_nelssa_max_n_centroids", 0),
        )
    ...
    # NELSSA: Cluster Metadata Transfer (Register NIXL memory region)
    def register_nelssa_metadata_buffers(
        self,
        centroid_buf: torch.Tensor,
        cluster_size_buf: torch.Tensor,
    ) -> None:
        self._nelssa_centroid_buf = centroid_buf
        self._nelssa_cluster_size_buf = cluster_size_buf
        self._nelssa_max_n_centroids = int(centroid_buf.shape[-2])

        # Register each staging buffer as a single contiguous NIXL region.
        descs = self.nixl_wrapper.get_reg_descs([centroid_buf, cluster_size_buf], self.nixl_memory_type)
        assert descs is not None
        self.nixl_wrapper.register_memory(descs, backends=self.nixl_backends)
        self._nelssa_registered_descs = descs
        self._nelssa_cluster_transfer_enabled = True

        slot_elems_centroid = int(
            centroid_buf.shape[1] * centroid_buf.shape[2]
            * centroid_buf.shape[3] * centroid_buf.shape[4]
        )
        slot_elems_cluster_size = int(
            cluster_size_buf.shape[1] * cluster_size_buf.shape[2]
            * cluster_size_buf.shape[3]
        )
        centroid_slot_bytes = slot_elems_centroid * centroid_buf.element_size()
        cluster_size_slot_bytes = slot_elems_cluster_size * cluster_size_buf.element_size()

        self._nelssa_centroid_base_addrs = [int(centroid_buf.data_ptr())]
        self._nelssa_cluster_size_base_addrs = [int(cluster_size_buf.data_ptr())]
        self._nelssa_centroid_slot_bytes = centroid_slot_bytes      # Stride
        self._nelssa_cluster_size_slot_bytes = cluster_size_slot_bytes
        logger.info(
            "[NIXL][NELSSA][META] Registered clustering metadata staging "
            "buffers: centroid %s, cluster_size %s (max_n_centroids=%d, "
            "kv_role=%s)",
            tuple(centroid_buf.shape),
            tuple(cluster_size_buf.shape),
            self._nelssa_max_n_centroids,
    ...
    def add_remote_agent(
        ...
        # NELSSA: Cluster Metadata Transfer
        # Store the Cluster Metadata Buffer (Prefill worker) info to construct NIXL READ descriptor
        if (
            self._nelssa_cluster_transfer_enabled
            and nixl_agent_meta.nelssa_centroid_slot_bytes > 0
            and remote_tp_rank == 0
        ):
            self._nelssa_meta_remote[engine_id] = (
                list(nixl_agent_meta.nelssa_centroid_base_addrs),
                list(nixl_agent_meta.nelssa_cluster_size_base_addrs),
                int(nixl_agent_meta.nelssa_centroid_slot_bytes),
                int(nixl_agent_meta.nelssa_max_n_centroids),
                int(nixl_agent_meta.device_id),
            )
            logger.info(
                "[NIXL][NELSSA][META] Cached remote metadata buffer geometry "
                "for engine %s (max_n_centroids=%d, device_id=%d)",
                engine_id,
                nixl_agent_meta.nelssa_max_n_centroids,
    ...
    def get_finished(self) -> tuple[set[str], set[str]]:
        ...
        # After NIXL READ completes, store data in runner & trigger slot free API
        done_meta = self._pop_done_transfers(self._recving_meta_transfers)
        for req_id in done_meta:
            self._on_nelssa_metadata_received(req_id)
    ...
    def set_nelssa_metadata_store_callback(self, callback: Callable) -> None:
        """Register the model-runner callback that stores received metadata."""
        self._nelssa_store_metadata_callback = callback

    def set_nelssa_meta_slot_free_callback(
        self, callback: Callable[[str, int], None]
    ) -> None:
        """Register callback to notify runner when NIXL READ completes."""
        self._nelssa_meta_slot_free_callback = callback

    # [Decode Worker] After NIXL READ finish, extract validate data from Staging Buffer
    # Store the slicing data in Metadata Buffer by using runner._store_received_nelssa_metadata
    def _on_nelssa_metadata_received(self, req_id: ReqId) -> None:
        if self._nelssa_store_metadata_callback is None:
            return

        info = self._nelssa_meta_recv_info.get(req_id)
        if info is None:
            logger.warning("[NIXL][NELSSA][META] _on_nelssa_metadata_received: no (slot, n_centroids)")
            return
        slot, n_centroids = info

        local_centroid = self._nelssa_centroid_buf
        local_cluster_size = self._nelssa_cluster_size_buf

        # Slice the slot and the valid n_centroids range.
        centroid_slot = local_centroid[slot, :, :, :n_centroids, :]
        cluster_size_slot = local_cluster_size[slot, :, :, :n_centroids]
        try:
            self._nelssa_store_metadata_callback(       # Save in Decode Metadata Buffer for similarity search
                req_id, centroid_slot, cluster_size_slot, n_centroids, slot
            )
            logger.info("[NIXL][NELSSA][META] Stored received metadata for %s (slot=%d, n_centroids=%d)", req_id, slot, n_centroids)
        except Exception as e:
            logger.warning("[NIXL][NELSSA][META] Store callback failed for %s: %s", req_id, e)

        # Consume the side-store entry now that the READ is fully handled.
        info = self._nelssa_meta_recv_info.pop(req_id, None)

        # Notify runner to send slot_free RPC to P-side (READ complete, slot can be freed)
        if self._nelssa_meta_slot_free_callback is not None and info is not None:
            try:
                self._nelssa_meta_slot_free_callback(req_id, info[0])
                logger.debug("[NIXL][NELSSA][META] Slot free callback invoked for %s slot=%d", req_id, info[0])
            except Exception as e:
                logger.warning("[NIXL][NELSSA][META] Slot free callback failed for %s: %s", req_id, e)
    ...
    # [Decode Worker] Build NIXL Descriptor & Start Cluster Metadata Transfer with NIXL READ
    def start_cluster_metadata_read(self, req_id: ReqId, meta: ReqMeta) -> None:
        """Issue a NIXL READ to pull NELSSA clustering metadata from the Prefill worker"""
        assert self._nelssa_cluster_transfer_enabled is True
        assert meta.remote is not None

        # Short (non-NELSSA) requests do not carry clustering metadata: they
        # have no staging slot on the Prefill side, so remote_slot_idx /
        # remote_n_centroids are None. Only Long requests go through NELSSA
        # (clustering / KV offload / similarity search / CPU sparse attention);
        # Short requests must flow through the original vLLM path. Skip the
        # cluster metadata READ here so a mixed Short+Long batch does not crash
        # on `slot_idx * local_centroid_slot_bytes` below.
        if meta.remote.slot_idx is None or meta.remote.n_centroids is None:
            logger.info(
                "[NIXL][NELSSA][META] %s is SHORT -> skip cluster metadata READ "
                "(no staging slot: slot_idx=%s, n_centroids=%s)",
                req_id[:25], meta.remote.slot_idx, meta.remote.n_centroids)
            return

        slot_idx = meta.remote.slot_idx
        engine_id = meta.remote.engine_id
        remote_geo = self._nelssa_meta_remote.get(engine_id)        # Cluster Metadata Info in Handshake
        remote_centroid_addrs, remote_cluster_size_addrs, remote_centroid_slot_bytes, remote_max_n_centroids, remote_device_id = remote_geo

        local_centroid = self._nelssa_centroid_buf
        local_cluster_size = self._nelssa_cluster_size_buf

        # Local slot byte strides.
        slot_elems_centroid = int(
            local_centroid.shape[1] * local_centroid.shape[2]
            * local_centroid.shape[3] * local_centroid.shape[4]
        )
        slot_elems_cluster_size = int(
            local_cluster_size.shape[1]
            * local_cluster_size.shape[2]
            * local_cluster_size.shape[3]
        )
        local_centroid_slot_bytes = slot_elems_centroid * local_centroid.element_size()
        local_cluster_size_slot_bytes = slot_elems_cluster_size * local_cluster_size.element_size()

        # Build (addr, len, device) descriptors for the slot slice of each buffer.
        local_centroid_addr = int(local_centroid.data_ptr()) + slot_idx * local_centroid_slot_bytes
        local_cluster_size_addr = int(local_cluster_size.data_ptr()) + slot_idx * local_cluster_size_slot_bytes
        remote_centroid_addr = int(remote_centroid_addrs[0]) + slot_idx * remote_centroid_slot_bytes
        remote_cluster_size_addr = int(remote_cluster_size_addrs[0]) + slot_idx * local_cluster_size_slot_bytes

        local_blocks_data = [
            (local_centroid_addr, local_centroid_slot_bytes, self.device_id),
            (local_cluster_size_addr, local_cluster_size_slot_bytes, self.device_id),
        ]
        remote_blocks_data = [
            (remote_centroid_addr, local_centroid_slot_bytes, remote_device_id),
            (remote_cluster_size_addr, local_cluster_size_slot_bytes, remote_device_id),
        ]

        try:
            local_descs = self.nixl_wrapper.get_xfer_descs(local_blocks_data, self.nixl_memory_type)
            remote_descs = self.nixl_wrapper.get_xfer_descs(remote_blocks_data, self.nixl_memory_type)
            remote_agent_name = self._remote_agents[engine_id][list(self._remote_agents[engine_id].keys())[0]]
            notif_id = f"{meta.remote.request_id}:meta:{self.world_size}".encode()
            ...
            self._recving_meta_transfers[req_id].append(handle)
            self._nelssa_meta_recv_info[req_id] = (slot_idx, meta.remote.n_centroids or 0)
            logger.info(
                "[NIXL][NELSSA][META] %s is LONG -> READ posted slot=%d "
                "(n_centroids=%d, max=%d)",
                req_id[:25], slot_idx, meta.remote.n_centroids or 0,
                remote_max_n_centroids)

        except Exception as e:
            logger.warning("[NIXL][NELSSA][META] Failed to post metadata READ for %s: %s", req_id, e)
    ...
    def start_load_kv(self, metadata: NixlConnectorMetadata):
        ...
        for req_id, meta in metadata.reqs_to_recv.items():
            ...
            if len(meta.local_block_ids[0]) < len(meta.remote.block_ids[0]):
                _compacted_remote = [
                    tuple(b for b in grp if int(b) != 0) or ()
                    for grp in meta.remote.block_ids
                ]
                meta.remote.block_ids = tuple(_compacted_remote)
            ...
            # NELSSA: Pull clustering metadata (centroids + cluster_size)
            if self._nelssa_cluster_transfer_enabled:
                self.start_cluster_metadata_read(req_id, meta)
        ...
        for req_id in metadata.reqs_not_processed:
            ...
            # Drop any in-flight NELSSA metadata READ info for the aborted req.
            self._nelssa_meta_recv_info.pop(req_id, None)
    ...
    def shutdown(self):
        ...
        for handles in self._recving_meta_transfers.values():
            for handle in handles:
                self.nixl_wrapper.release_xfer_handle(handle)
        self._recving_meta_transfers.clear()
        self._nelssa_meta_recv_info.clear()
