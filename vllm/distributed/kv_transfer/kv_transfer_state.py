...
_NELSSA_META_SLOT_REGISTRY: dict[str, tuple[int, int]] = {}


def record_nelssa_meta_slot(req_id: str, slot: int, n_centroids: int) -> None:
    """Record the staging slot a request's NELSSA metadata was staged into."""
    _NELSSA_META_SLOT_REGISTRY[req_id] = (slot, n_centroids)


def pop_nelssa_meta_slot(req_id: str) -> tuple[int, int] | None:
    """Return and clear the staged (slot, n_centroids) for a request."""
    return _NELSSA_META_SLOT_REGISTRY.pop(req_id, None)
