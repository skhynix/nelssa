...
class EngineArgs:
    ...
    nelssa_config: NELSSAConfig = field(default_factory=NELSSAConfig)
    # Individual NELSSA CLI fields (will be merged into nelssa_config in post_init)
    nelssa_enabled: bool | None = None
    nelssa_first_n: int | None = None
    nelssa_last_m: int | None = None
    nelssa_prompt_length_threshold: int | None = None
    nelssa_nprobe: int | None = None
    nelssa_es_cluster_num: int | None = None
    nelssa_retrieval_budget: float | None = None
    nelssa_estimation_budget: float | None = None
    nelssa_enable_attention: bool | None = None
    nelssa_cpu_attention_kv_fraction: float | None = None
    nelssa_enable_offloading: bool | None = None
    nelssa_max_num_long_requests: int | None = None
    ...
    def __post_init__(self):
        ...
        if isinstance(self.nelssa_config, dict):
            self.nelssa_config = NELSSAConfig(
                **self.nelssa_config
            )
        # Merge individual CLI nelssa_* fields into nelssa_config
        if self.nelssa_enabled is True:
            self.nelssa_config.enabled = True
            # Note: enable_cpu_attention CLI arg is named 'nelssa_enable_attention' (not 'nelssa_enable_cpu_attention')
            field_mapping = {
                'first_n': 'nelssa_first_n',
                'last_m': 'nelssa_last_m',
                'prompt_length_threshold': 'nelssa_prompt_length_threshold',
                'nprobe': 'nelssa_nprobe',
                'es_cluster_num': 'nelssa_es_cluster_num',
                'retrieval_budget': 'nelssa_retrieval_budget',
                'estimation_budget': 'nelssa_estimation_budget',
                'enable_cpu_attention': 'nelssa_enable_attention',  # CLI arg name differs from config field name
                'cpu_attention_kv_fraction': 'nelssa_cpu_attention_kv_fraction',
                'enable_offloading': 'nelssa_enable_offloading',
                'max_num_long_requests': 'nelssa_max_num_long_requests',
            }
            for config_field_name, cli_attr_name in field_mapping.items():
                cli_value = getattr(self, cli_attr_name, None)
                if cli_value is not None:
                    setattr(self.nelssa_config, config_field_name, cli_value)
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        # NELSSA configuration arguments
        nelssa_kwargs = get_kwargs(NELSSAConfig)
        nelssa_group = parser.add_argument_group(
            title="NELSSAConfig",
            description=NELSSAConfig.__doc__,
        )
        nelssa_group.add_argument(
            "--nelssa-enabled",
            **nelssa_kwargs["enabled"],
        )
        nelssa_group.add_argument(
            "--nelssa-first-n",
            **nelssa_kwargs["first_n"],
        )
        nelssa_group.add_argument(
            "--nelssa-last-m",
            **nelssa_kwargs["last_m"],
        )
        nelssa_group.add_argument(
            "--nelssa-prompt-length-threshold",
            **nelssa_kwargs["prompt_length_threshold"],
        )
        nelssa_group.add_argument(
            "--nelssa-nprobe",
            **nelssa_kwargs["nprobe"],
        )
        nelssa_group.add_argument(
            "--nelssa-es-cluster-num",
            **nelssa_kwargs["es_cluster_num"],
        )
        nelssa_group.add_argument(
            "--nelssa-retrieval-budget",
            **nelssa_kwargs["retrieval_budget"],
        )
        nelssa_group.add_argument(
            "--nelssa-estimation-budget",
            **nelssa_kwargs["estimation_budget"],
        )
        nelssa_group.add_argument(
            "--nelssa-enable-attention",
            **nelssa_kwargs["enable_cpu_attention"],
        )
        nelssa_group.add_argument(
            "--nelssa-cpu-attention-kv-fraction",
            **nelssa_kwargs["cpu_attention_kv_fraction"],
        )
        nelssa_group.add_argument(
            "--nelssa-enable-offloading",
            **nelssa_kwargs["enable_offloading"],
        )
        nelssa_group.add_argument(
            "--nelssa-max-num-long-requests",
            type=int,
            default=None,
        )
    ...
    def create_engine_config(
        ...
        cache_config = CacheConfig(
            ...
            nelssa_config=self.nelssa_config if self.nelssa_config is not None else NELSSAConfig(),
        )
