...
class FlashAttentionImpl(AttentionImpl):
    ...
    @staticmethod
    def _merge_lse_splits(lse_splits: torch.Tensor) -> torch.Tensor:
        # NELSSA: Numerically-stable logsumexp merge over the split axis.
        lf = lse_splits.float()
        lf = lf.masked_fill(~torch.isfinite(lf), float("-inf"))
        lse_max = lf.max(dim=-1, keepdim=True).values
        lse_max = torch.where(torch.isfinite(lse_max), lse_max, torch.zeros_like(lse_max))
        merged = lse_max + torch.log(torch.exp(lf - lse_max).sum(dim=-1, keepdim=True))
        return merged.clamp_min(torch.finfo(torch.float32).min)
    ...
    def forward(
        ...
        if not attn_metadata.use_cascade:
            ...
            if self.dcp_world_size > 1:
                ...
                # Call flash_attn_varlen_func with return_softmax_lse to get LSE
                if lse is not None:
                    # Request LSE output
                    ...
                    # flash_output is (output, lse) when return_softmax_lse=True
                    if isinstance(flash_output, tuple):
                        _, lse_from_flash = flash_output
                        # Normalize flash_attn's LSE into NELSSA's [batch, num_heads] buffer. 
                        if lse_from_flash.dim() == 3:       # logsumexp-merge over the split axis
                            lse.copy_(self._merge_lse_splits(lse_from_flash))
                        elif lse_from_flash.dim() == 2:
                            lse.copy_(lse_from_flash.t().contiguous())
                        else:
                            lse.copy_(lse_from_flash.view(lse.shape))
                else:
                    # No LSE requested
                    flash_attn_varlen_func(
                        q=query[:num_actual_tokens],
