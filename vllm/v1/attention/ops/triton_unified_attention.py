...
def kernel_unified_attention(
    ...
    for j in range(loop_lo, loop_hi):
        ...
        # Compute block index first
        block_index = seq_offset // BLOCK_SIZE

        # Debug: print seq_offset info for first tile only
        # if q_block_global_idx == 0 and kv_head_idx == 0 and segm_idx > 6:
        #     tl.device_print("seq_offset", seq_offset)
        #     tl.device_print("block_index", block_index)

        physical_block_idx = tl.load(
            block_tables_ptr + block_table_offset + block_index
        ).to(tl.int64)

        # Debug: print physical block idx after load (first tile only)
        # if q_block_global_idx == 0 and kv_head_idx == 0 and segm_idx > 6:
        #     tl.device_print("physical_block_idx", physical_block_idx)
