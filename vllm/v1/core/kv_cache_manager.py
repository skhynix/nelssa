...
class KVCacheManager:
    ...
    def allocate_slots(
        ...
        # Todo: preprocess (SparseKVCacheManager)
        # Mark as a long request in the coordinator
        nelssa_long_mask = getattr(request, 'nelssa_long_mask', False)
        if nelssa_long_mask:
            self._mark_long_request(request.request_id, request.num_tokens)
        ...
        new_blocks = self.coordinator.allocate_new_blocks(
            request.request_id,
            num_tokens_need_slot,           # number of tokens to allocate slots
            num_tokens_main_model,          # number of tokens to process by model
            num_encoder_tokens,
        )
    ...
    # SparseKVCacheManager가 해당 long request를 별도로 관리할 수 있도록 마킹 (single_type_kv_cache_manager.py)
    def _mark_long_request(self, request_id: str, prompt_len: int) -> None:
        for mgr in self.coordinator.single_type_managers:
            if hasattr(mgr, 'mark_long_request'):
                mgr.mark_long_request(request_id, prompt_len)

    # Todo: clustering (freeing)
    # Free middle GPU blocks after clustering completes for long requests.
    def free_sparse_kv(self, request_id: str) -> None:
        for mgr in self.coordinator.single_type_managers:
            if hasattr(mgr, 'free_middle_blocks'):  # SparseKVCacheManager
                mgr.free_middle_blocks(request_id)
