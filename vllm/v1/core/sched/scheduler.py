...
from vllm.config import NELSSAConfig, VllmConfig
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        self.nelssa_config = self.cache_config.nelssa_config
        ...
        # Todo: clustering
        # Track requests that completed prefill in this step
        self._prefill_complete_reqs: list[str] = []

        # Todo: stats logging
        # 5-second interval stats logging for WAITING/Preempted requests
        self._last_sched_log_time: float = time.time()
        ...
        self.kv_cache_manager = KVCacheManager(
            ...
            nelssa_config=self.nelssa_config,
        )
        ...
        # NELSSA: Max Long Requests
        self.max_num_long_requests = self.nelssa_config.max_num_long_requests
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        # Reset prefill complete tracking for this step
        self._prefill_complete_reqs = []
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            with record_function_or_nullcontext("schedule: allocate_slots"):
                while True:
                    ...
                    nelssa_protected_skipped = 0
                    nelssa_preempt_fallback = False
                    if self.policy == SchedulingPolicy.PRIORITY:
                        # NELSSA: Avoid Long Request Preemption
                        # Prefer preempting a Short (or a not-yet-prefilled) request. 
                        preemptable = [r for r in self.running if not self._is_nelssa_preempt_protected(r)]
                        nelssa_protected_skipped = len(self.running) - len(preemptable)
                        if preemptable:
                            preempted_req = max(preemptable, key=lambda r: (r.priority, r.arrival_time))
                        else:
                            nelssa_preempt_fallback = True
                        ...
                        for i in range(len(self.running) - 1, -1, -1):
                            cand = self.running[i]
                            if self._is_nelssa_preempt_protected(cand):
                                nelssa_protected_skipped += 1
                                continue
                            preempted_req = self.running.pop(i)
                            break
                        nelssa_preempt_fallback = preempted_req is None
                        if preempted_req is None:
                            # All running requests are protected Long-decode reqs.
                            # Fall back to the FCFS tail
                            preempted_req = self.running.pop()

                    self._preempt_request(preempted_req, scheduled_timestamp)
                    preempted_reqs.append(preempted_req)
                    # Debug: Log preempt immediately
                    print(f"[Scheduler] PREEMPTED: {preempted_req.request_id} (total preempted so far: {len(preempted_reqs)})", flush=True)
        ...
        if not preempted_reqs and self._pause_state == PauseState.UNPAUSED:
            ...
            while (self.waiting or self.skipped_waiting) and token_budget > 0:
                ...
                # Select waiting queue according to scheduling policy (waiting vs skipped_waiting)
                ...
                # NELSSA: Max Long Requests
                if self.max_num_long_requests > 0:
                    num_running_long_requests = sum(1 for req in self.running if getattr(req, 'nelssa_long_mask', False))

                    # NELSSA: Preprocess
                    if self.nelssa_config is not None and self.nelssa_config.nelssa_check(request.num_prompt_tokens):
                        request.nelssa_long_mask = True
                        request.head_size = self.nelssa_config.first_n
                        request.tail_size = self.nelssa_config.last_m

                    if request.nelssa_long_mask and num_running_long_requests >= self.max_num_long_requests:
                        step_skipped_waiting.prepend_request(request_queue.pop_request())
                        continue
                ...
                # HS: P/D disaggregate (KV Cache Transfer)
                # Case 1: WAITING decode requests / prefill requests
                if request.num_computed_tokens == 0:
                    # Get locally-cached tokens.(Local Prefix Caching)
                    ...
                    # Case 2: RUNNING decode requests
        ...
        # NELSSA: preprocess (SchedulerOutput prepare)
        # KV cache offload info - simplified: offload all tokens in prefill chunks (include head/tail)
        nelssa_long_mask_map = {}
        head_size_map = {}
        tail_size_map = {}
        chunk_info_map = {}

        if self.nelssa_config is not None:
            all_scheduled_reqs = scheduled_new_reqs + scheduled_running_reqs + scheduled_resumed_reqs
            for req_idx, req in enumerate(all_scheduled_reqs):
                nelssa_long_mask_map[req.request_id] = getattr(req, 'nelssa_long_mask', False)
                head_size_map[req.request_id] = getattr(req, 'head_size', None)
                tail_size_map[req.request_id] = getattr(req, 'tail_size', None)

                num_scheduled = num_scheduled_tokens.get(req.request_id, 0)
                num_computed_before = req.num_computed_tokens
                num_computed_after = req.num_computed_tokens + num_scheduled
                prompt_len = req.num_tokens

                if getattr(req, 'nelssa_long_mask', False):
                    # is_prefill_chunk is True only if we're scheduling prompt tokens (not generated tokens) >> Clustering init flag
                    is_prefill_chunk = num_computed_before < req.num_prompt_tokens
                    max_total_len = prompt_len
                    if req.sampling_params and hasattr(req.sampling_params, 'max_tokens'):
                        max_total_len = prompt_len + (req.sampling_params.max_tokens or 0)
                    chunk_info_map[req.request_id] = {
                        "is_prefill_chunk": is_prefill_chunk,
                        "prompt_len": prompt_len,
                        "max_total_len": max_total_len,
                        "chunk_start": num_computed_before,  # Start token index of this chunk
                        "chunk_end": num_computed_after,     # End token index of this chunk
                        "req_idx": req_idx,                  # Index matching query_start_loc order (offloading)
                    }

                    if num_computed_after >= prompt_len and is_prefill_chunk:
                        self._prefill_complete_reqs.append(req.request_id)

        prefill_complete_reqs = getattr(self, '_prefill_complete_reqs', [])
        ...
        scheduler_output = SchedulerOutput(
            ...
            nelssa_long_mask=nelssa_long_mask_map,
            head_size=head_size_map,
            tail_size=tail_size_map,
            chunk_info=chunk_info_map,
            prefill_complete_reqs=prefill_complete_reqs,
        )
        ...
        # Log WAITING/Preempted stats every 5 seconds
        current_time = time.time()
        if current_time - self._last_sched_log_time >= 5.0:
            self._last_sched_log_time = current_time
            num_waiting = len(self.waiting)
            num_running = len(self.running)
            print(f"[Scheduler] STATS: Running={num_running} | "
                  f"Waiting={num_waiting}", flush=True)
    ...
    def _is_nelssa_preempt_protected(self, request: Request) -> bool:
        if self.nelssa_config is None:
            return False
        if not getattr(request, 'nelssa_long_mask', False):
            return False
    ...
    def _update_after_schedule(self, scheduler_output: SchedulerOutput) -> None:
        ...
        for req_id, num_scheduled_token in num_scheduled_tokens.items():
            ...
            before_update = request.num_computed_tokens
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            # Free middle blocks for long requests that completed prefill
            # free_sparse_kv internally calls free_middle_blocks for SparseKVCacheManager
            prefill_complete_reqs = getattr(scheduler_output, 'prefill_complete_reqs', [])
            if self.kv_cache_manager and req_id in prefill_complete_reqs:
                self.kv_cache_manager.free_sparse_kv(req_id)
    ...
    def _select_waiting_queue_for_scheduling(self) -> RequestQueue | None:
        if self.policy == SchedulingPolicy.FCFS:
            if self.skipped_waiting:
                ...
                if getattr(skipped_req, 'nelssa_long_mask', False):
        ...
        if self.waiting and self.skipped_waiting:
            ...
            waiting_is_long = getattr(waiting_req, 'nelssa_long_mask', False)
            skipped_is_long = getattr(skipped_req, 'nelssa_long_mask', False)

            if skipped_is_long and not waiting_is_long:
                return self.skipped_waiting
            if waiting_is_long and not skipped_is_long:
                return self.waiting
