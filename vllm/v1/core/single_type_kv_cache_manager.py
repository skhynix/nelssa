...
from vllm.config import NELSSAConfig
...
def get_manager_for_kv_cache_spec(
    ...
    **kwargs,
    ...
    nelssa_config = kwargs.pop("nelssa_config", None)
    if nelssa_config is not None:
        return NELSSACacheManager(
            kv_cache_spec=kv_cache_spec,
            block_pool=kwargs["block_pool"],
            enable_caching=kwargs["enable_caching"],
            kv_cache_group_id=kwargs["kv_cache_group_id"],
            dcp_world_size=kwargs.get("dcp_world_size", 1),
            pcp_world_size=kwargs.get("pcp_world_size", 1),
            max_admission_blocks_per_request=kwargs.get("max_admission_blocks_per_request"),
            nelssa_config=nelssa_config,
        )
...
# Todo: preprocess (NELSSACacheManager)
class NELSSACacheManager(FullAttentionManager):
    def __init__(
        self,
        kv_cache_spec: KVCacheSpec,
        block_pool: BlockPool,
        ...
        pcp_world_size: int = 1,
        max_admission_blocks_per_request: int | None = None,
        nelssa_config: NELSSAConfig | None = None,
        device: str = "cuda",
    ) -> None:
        super().__init__(
            kv_cache_spec,
        ...
        self.nelssa_config = nelssa_config
        self.head_size = nelssa_config.first_n if nelssa_config else 16
        self.tail_size = nelssa_config.last_m if nelssa_config else 64
        self.device = device
        self.long_requests: set[str] = set()
        self.nelssa_tail_blocks: dict[str, int] = {}
        self.nelssa_tail_start_pos: dict[str, int] = {}

    def _compute_tail_layout(self, prompt_len: int) -> tuple[int, int]:
        block_size = self.block_size
        tail_size = self.tail_size
        tail_start_pos = max(prompt_len - tail_size, 0)
        
        tail_start_pos = (tail_start_pos // block_size) * block_size
        tail_tokens = max(prompt_len - tail_start_pos, 0)
        tail_blocks = (tail_tokens + block_size - 1) // block_size
        return tail_blocks, tail_start_pos

    def mark_long_request(self, request_id: str, prompt_len: int) -> None:
        self.long_requests.add(request_id)
        tail_blocks, tail_start_pos = self._compute_tail_layout(prompt_len)
        self.nelssa_tail_blocks[request_id] = tail_blocks
        self.nelssa_tail_start_pos[request_id] = tail_start_pos

    def nelssa_long_mask(self, request_id: str) -> bool:
        return request_id in self.long_requests
    ...
    def get_num_blocks_to_allocate(
        ...
        if request_id in self.long_requests:
            head_blocks = cdiv(self.head_size, self.block_size)
            tail_blocks = self.nelssa_tail_blocks.get(request_id, cdiv(self.tail_size, self.block_size))
            num_required = head_blocks + tail_blocks + cdiv(
                max(num_tokens - total_computed_tokens, 0), self.block_size)
            num_req_blocks = len(self.req_to_blocks.get(request_id, ()))
            return max(num_required - num_req_blocks, 0)
        return super().get_num_blocks_to_allocate(
            request_id,
            num_tokens,
            new_computed_blocks,
    ...
    # Todo: clustering (freeing)
    def free_middle_blocks(self, request_id: str) -> None:
        """Free middle blocks after prefill, keeping only head/tail blocks on GPU."""
        if request_id not in self.long_requests:
            return
        ...
        tail_blocks = self.nelssa_tail_blocks.get(request_id, (tail_size + block_size - 1) // block_size)

        total_blocks = len(blocks)
        middle_start_idx = head_blocks
        middle_end_idx = total_blocks - tail_blocks if tail_blocks > 0 else total_blocks
        blocks_to_free = middle_end_idx - middle_start_idx

        if blocks_to_free > 0:
            # Phase 1: Collect blocks to free (validate ref_cnt > 0 to avoid double-free)
            blocks_to_free_list = []
            for i in range(middle_start_idx, middle_end_idx):
                block = blocks[i]
                if block != self._null_block and block.block_id >= 0 and block.ref_cnt > 0:
                    blocks_to_free_list.append((i, block))

            # Phase 2: Bulk free (decrease ref_cnt once for all blocks)
            if blocks_to_free_list:
                blocks_batch = [b for _, b in blocks_to_free_list]
                self.block_pool.free_blocks(blocks_batch)

                # Phase 3: Update CPU block table after free completes
                for i, _ in blocks_to_free_list:
                    blocks[i] = self._null_block

    def free_request_resources(self, request_id: str) -> None:
        self.long_requests.discard(request_id)
        self.nelssa_tail_blocks.pop(request_id, None)
        self.nelssa_tail_start_pos.pop(request_id, None)
