...
class EngineCore:
    ...
    def step_with_batch_queue(
        ...
        """
        # logger.info("================== step_with_batch_queue (core.py) ==================")
...
class EngineCoreProc(EngineCore):
    ...
    def run_engine_core(*args, dp_rank: int = 0, local_dp_rank: int = 0, **kwargs):
        ...
        try:
            ...
            # NELSSA EngineCore core isolation (EC-PIN). When NELSSA_ENGINE_CORES
            # is set (e.g. by run_pd_with_nelssa.sh from NELSSA_EC_NCORES), pin
            # THIS EngineCore process to those cores via sched_setaffinity so it
            # does NOT run on the CPU-attention OMP cores (72,80,...,124 by
            # default) that the RPC-server thread's fused matmul needs. The CPU
            # attention is compute-bound (AMX throughput), so EngineCore
            # stealing AMX cycles on those cores slows fused ~1.4x. This keeps
            # the engine off those cores entirely. The RPC server thread itself
            # is pinned separately in gpu_model_runner (RPC-PIN).
            _nelssa_ec = os.environ.get("NELSSA_ENGINE_CORES", "").strip()
            if _nelssa_ec:
                try:
                    _ec_cores = sorted({int(c) for c in _nelssa_ec.split(",") if c.strip()})
                    if _ec_cores:
                        os.sched_setaffinity(0, _ec_cores)
                        logger.info(
                            "[NELSSA][EC-PIN] EngineCore pid=%s pinned to %d cores "
                            "(%s), excluding CPU-attn cores",
                            os.getpid(), len(_ec_cores), _nelssa_ec)
                except Exception as e:
                    logger.warning("[NELSSA][EC-PIN] affinity set failed: %s", e)
